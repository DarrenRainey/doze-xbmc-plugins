...
# plugin constants
__plugin__ = "SubNettiTV"
__author__ = "doze"
__url__ = "http://code.google.com/p/doze-xbmc-plugins/"
__svn_url__ = "http://doze-xbmc-plugins.googlecode.com/svn/trunk/SubNettiTV"
...
class SubNettiTV:
  ...
  def openUrl(self, url):
    ...
    try:
      ...
      dialog.ok("SubNettiTV - " +xbmc.getLocalizedString(257), 'Exception:' +'\n'+ str(value) +'\n'+ 'Data:' +'\n'+ str(data))
    if (data==""):
      dialog = xbmcgui.Dialog()
      dialog.ok("SubNettiTV - " +xbmc.getLocalizedString(257), xbmc.getLocalizedString(284)+ +'\n'+ 'CookieJar:'+ str(self.cookieJar))
      return
    return data
  ...
  def browsePrograms(self, url):
    ...
    rePattern = re.compile('<a onfocus="blur\(\);" href=\"([^\"]netti-tv\/index.shtml\/ohjelmat[^\"]+)">(.*?)<', re.IGNORECASE + re.DOTALL + re.MULTILINE)
  ...
  def browseEpisodes(self, url):
    data = self.openUrl(url)
    rePattern = re.compile('<a href=\"\/netti-tv\/index.shtml\/ohjelmat[^\?]+\?([^\"]+)\">(.*?)\<[^\(]+\(([^\)]+)', re.IGNORECASE + re.DOTALL + re.MULTILINE)
    matches = rePattern.findall(data)
    for id, name, date in matches:
      if (date != ''):
        try:
      ...
      ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url = 'http://www.sub.fi/id/'+id+'.asx',listitem=liz,isFolder=False,totalItems=len(matches))
...
if (params != ""):
  ...
  if (urllib.unquote_plus(param['action']) == "browse_programs"):
    ...
    s.browsePrograms('http://www.sub.fi/netti-tv/')
  #browse episodes
  elif (urllib.unquote_plus(param['action']) == "browse_episodes"):
    xbmcplugin.addSortMethod( handle=int( sys.argv[ 1 ] ), sortMethod=xbmcplugin.SORT_METHOD_LABEL )
    xbmcplugin.addSortMethod( handle=int( sys.argv[ 1 ] ), sortMethod=xbmcplugin.SORT_METHOD_DATE )
    url = unquote_safe(param.get('url', ''))
    if (url != ''):
      s.browseEpisodes('http://www.sub.fi'+url)
#no parameters set, open default view
else:
  liz=xbmcgui.ListItem(xbmc.getLocalizedString(30200),iconImage="DefaultVideo.png")
