"""
    Plugin for streaming free media from MTV3 anytime (Finnish media portal)
"""
...
# plugin constants
__plugin__ = "MTV3anytime"
__author__ = "doze"
__url__ = "http://code.google.com/p/doze-xbmc-plugins/"
__svn_url__ = "http://doze-xbmc-plugins.googlecode.com/svn/trunk/MTV3anytime"
...
class MTV3anytime:
  ...
  def openUrl(self, url):
    ...
    try:
      ...
      dialog.ok("MTV3anytime - " +xbmc.getLocalizedString(257), 'Exception:' +'\n'+ str(value) +'\n'+ 'Data:' +'\n'+ str(data))
    if (data==""):
      dialog = xbmcgui.Dialog()
      dialog.ok("MTV3anytime - " +xbmc.getLocalizedString(257), xbmc.getLocalizedString(284)+ +'\n'+ 'CookieJar:'+ str(self.cookieJar))
      return
    return data
  ...
  def browsePrograms(self, url):
    ...
    rePattern = re.compile('<a.*?nostolinkki\" href=\"([^\"]+)\">([^<]+)<', re.IGNORECASE + re.DOTALL + re.MULTILINE)
    matches = rePattern.findall(data)
    for url, name in matches:
      liz=xbmcgui.ListItem(clean1(clean2(clean3(smart_unicode(clean_name(name))))),iconImage="DefaultVideo.png")
      ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url = sys.argv[0] + "?action=browse_episodes&url="+quote_safe(url),listitem=liz,isFolder=True,totalItems=len(matches))
  ...
  def browseEpisodes(self, url):
    ...
    #rePattern = re.compile('<a.*?nostolinkki\" href=\"[^\?]+\?([^\#]+).*?\">([^<]+)<', re.IGNORECASE + re.DOTALL + re.MULTILINE)
    #matches = rePattern.findall(data)
    rePattern = re.compile('<a.*?onClick=\"play\(([^\)]+).*?>([^\<]+)', re.IGNORECASE + re.DOTALL + re.MULTILINE)
    matches = rePattern.findall(data)
    for id, name in matches:
      ...
      if (xbmcplugin.getSetting("display_date") == "true"):
        ...
        title = name
      liz=xbmcgui.ListItem(clean1(clean2(clean3(smart_unicode(clean_name(name))))),iconImage="DefaultVideo.png")
      liz.setInfo( "video", { "Title"        : clean1(clean2(clean3(smart_unicode(clean_name(name)))))})
      ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url = 'http://nettitv.mtv3.fi/id/'+id+'.vmw',listitem=liz,isFolder=False,totalItems=len(matches))
...
if (params != ""):
  ...
  if (urllib.unquote_plus(param['action']) == "browse_programs"):
    ...
    m.browsePrograms('http://nettitv.mtv3.fi/nettitv_uutiset/links.shtml/mtv3/erikoisnostot/kaikkiohjelmat/')
  #browse episodes
  elif (urllib.unquote_plus(param['action']) == "browse_episodes"):
    xbmcplugin.addSortMethod( handle=int( sys.argv[ 1 ] ), sortMethod=xbmcplugin.SORT_METHOD_LABEL )
    #xbmcplugin.addSortMethod( handle=int( sys.argv[ 1 ] ), sortMethod=xbmcplugin.SORT_METHOD_DATE )
    url = param.get('url', '')
    #if url is given, make it to episode links url and parse
    if (url != ''):
      url = unquote_safe(url).replace('/nettitvuutiset/index.shtml/','/nettitv_uutiset/links.shtml/')
      url = unquote_safe(url).replace('/maksuttomat/index.shtml/','/maksuttomat/links.shtml/')
      m.browseEpisodes('http://nettitv.mtv3.fi'+url)
#no parameters set, open default view
else:
  liz=xbmcgui.ListItem(xbmc.getLocalizedString(30100),iconImage="DefaultVideo.png")
